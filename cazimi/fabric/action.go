...
func (action *Action) Initialize() error {
	...
	if config.Config().SelectionProvider != config.AutoDetectSelectionProvider {
	...
	sdk, err := fabsdk.New(config.Config().ConfigProvider, opts...)
	if err != nil {
	...
	if config.Config().IsLoggingEnabledFo {
		log.Debug("All Peers:")
	...
	peers, err := action.getPeers(allPeers, config.Config().PeerURLs, config.Config().OrgIDs)
	if err != nil {
		return err
	}
	...
	action.printer = printer.NewBlockPrinterWithOpts(
		printer.AsOutputFormat(config.Config().PrintFormat),
		printer.AsWriterType(config.Config().Writer),
		&printer.FormatterOpts{Base64Encode: config.Config().Base64})
...
func (action *Action) ChannelClient(...channel.ClientOption) (*channel.Client, error) {
	...
	channelProvider := func() (context.Channel, error) {
		return contextImpl.NewChannel(session, config.Config().ChannelID)
	}
...
func (action *Action) Orderers() ([]fab.Orderer, error) {
	...
	ordererURL := config.Config().OrdererURL
